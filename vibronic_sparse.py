...
def main(model, plotting=False):
 ...
 # basis sizes (store in dictionary for easy passing to functions)
    ...
    if model=='Displaced':
        param_times_grid2=displaced['lambda']*grid2
    if model=='Jahn_Teller':
    ...
    #output files
        rho1_out=open('h1_T'+str(T[tindex])+'.dat','w')
        rho2_out=open('h2_T'+str(T[tindex])+'.dat','w')
        rho12_out=open('h12_T'+str(T[tindex])+'.dat','w')
        w12_out=open('w12_T'+str(T[tindex])+'.dat','w')
        rhoa_out=open('a_T'+str(T[tindex])+'.dat','w')
        ...
        rho1=(1./Z[tindex])*rho1
        rho2=(1./Z[tindex])*rho2
        rho12=(1./Z[tindex])*rho12
        rhoa=(1./Z[tindex])*rhoa

        w12=-T[tindex]*eV_per_K*np.log(rho12)
